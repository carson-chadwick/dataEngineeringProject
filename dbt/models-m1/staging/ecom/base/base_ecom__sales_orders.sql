-- Purpose: This base model extracts raw legacy sales order data from JSON.
-- Role: It fits into the project by providing the initial layer of structure for historical e-commerce sales.

with

source as (
    -- Retrieve raw JSON sales orders
    select * from {{ source('ecom_source', 'sales_order_raw') }}

),

renamed as (
    -- Extract and cast all relevant sales fields from the raw JSON variant
    select
...
-- Output structured base sales orders
select * from renamed
